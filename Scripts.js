...
class AminaCasino {
    ...
    async initWallet() {
        ...
        try {
            if (typeof PeraWalletConnect === 'undefined') {
                throw new Error('PeraWalletConnect not loaded. Please refresh the page.');
            }

            // Single instance for both mobile and desktop
            this.peraWallet = new PeraWalletConnect({
                shouldShowSignTxnToast: false,
                chainId: 416002  // Use 416002 for TestNet or 416001 for MainNet
            });
        ...
        if (savedAccount && this.peraWallet) {
            try {
                ...
                if (accounts?.length > 0) {
                    ...
                    console.log('✅ Reconnected to saved wallet');
                    return true;
                }
            } catch (error) {
    ...
    setupPegs(){
        ...
        const rows=10;
        const pegArea=canvas.width*0.75;
    ...
    async dropPlinko(){
        ...
        this.showGameResult('plinko',`Hit ${multiplier}x! ${winAmount>bet?'Won':'Lost'} ${Math.abs(winAmount-bet).toFixed(2)} ${this.currentCurrency}!`,winAmount>=bet?'win':'lose');
    ...
    animateBall(){
        return new Promise(resolve=>{
            const canvas=this.plinkoCtx.canvas;
            const ball={x:canvas.width/2,y:15,vx:0,vy:0,radius:4,gravity:0.2,bounce:0.3};
            const animate=()=>{
                this.drawBoard();
...
// Initialize the casino when DOM is ready
